...
function strConcat(bytes _a, string _b, bytes _c) internal returns (string){
    ...
    function authenticatePlayerName(string name) constant returns (string result) {
	result = "Successfully authenticated name against database";
	if( bytes(name).length != 4){
		result = "Incorrect name length";
	}
	// hint that MONEY is all caps
        bytes memory salt = "_MONEY}";
	...
	if (sha3(hash) == sha3('461f168de32239db6e3cfee433ea59a8d5cd5ec3')){
