...
host = argv[1] if len(argv) == 3 else '0.0.0.0'
port = argv[2] if len(argv) == 3 else 1234

from flask import Flask, render_template, g, request, redirect, flash, send_file, url_for, make_response
...
def verify(assignment_id):
    ...
    db.commit()
    response = make_response(render('verify.html', content=content))
    response.set_cookie('flag', value=flag)
    return response
...
def submit():
    ...
    run_chrome(f'/verify/{assignment_id}')
    return redirect('/')
